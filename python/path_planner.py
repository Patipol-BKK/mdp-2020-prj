...
def get_disc_collision_mask(obstacles):
...
@jit(nopython=True)
def discrete_pathfind(start_pos, end_pos, cost_grid, prev_grid, edges):
...
def get_distance_graph(obstacles, coll_grid, init_pos, edges):
    ...
    for i in range(len(obstacles) + 1):
        ...
        for j in range(0, len(obstacles)):
            ...
            cost, prev = discrete_pathfind(end_pos, start_pos, cost_grid, prev_grid, edges)
...
def generate_instr(_init_pos, _obstacles):
    ...
    start = time.time()
    coll_grid = get_disc_collision_mask(obstacles)
    edges = create_graph(coll_grid)
    print("Graph created - %.3fsec" % (time.time() - start))

    start = time.time()
    cost_matrix, path_matrix = get_distance_graph(obstacles, coll_grid, init_pos, edges)
