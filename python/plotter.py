...
def turn_right(pos, alpha):
    ...
    x_new = x_turning_center + \
        math.sin(math.radians(theta + alpha - 90)) * TURNING_R
    y_new = y_turning_center + \
        math.cos(math.radians(theta + alpha - 90)) * TURNING_R
...
def turn_left(pos, alpha):
    ...
    x_new = x_turning_center + \
        math.cos(math.radians(alpha - theta)) * TURNING_L
    y_new = y_turning_center + \
        math.sin(math.radians(alpha - theta)) * TURNING_L
    theta_new = (theta - alpha) % 360
...
def distance_to_collision(pos, obstacles):
    ...
    while not collided:
        x_front = cur_pos[0] + \
            math.sin(math.radians(cur_pos[2])) * (LENGTH - BACK_INSET)
        y_front = cur_pos[1] + \
            math.cos(math.radians(cur_pos[2])) * (LENGTH - BACK_INSET)
        for obstacle in obstacles:
            collided = point_is_collided(
                (x_front, y_front), obstacle[0], obstacle[1])
            if collided:
                break
        if not collided:
...
def get_outer_points(pos):
    x, y, theta = pos[0], pos[1], pos[2]
    x_left_front = x + math.sin(math.radians(theta)) * (LENGTH -
                                                        BACK_INSET) - math.cos(math.radians(theta)) * (WIDTH / 2)
    y_left_front = y + math.cos(math.radians(theta)) * (LENGTH -
                                                        BACK_INSET) + math.sin(math.radians(theta)) * (WIDTH / 2)

    x_right_front = x + math.sin(math.radians(theta)) * (LENGTH -
                                                         BACK_INSET) + math.cos(math.radians(theta)) * (WIDTH / 2)
    y_right_front = y + math.cos(math.radians(theta)) * (LENGTH -
                                                         BACK_INSET) - math.sin(math.radians(theta)) * (WIDTH / 2)

    x_left_back = x - math.sin(math.radians(theta)) * \
        BACK_INSET - math.cos(math.radians(theta)) * (WIDTH / 2)
    y_left_back = y - math.cos(math.radians(theta)) * \
        BACK_INSET + math.sin(math.radians(theta)) * (WIDTH / 2)

    x_right_back = x - math.sin(math.radians(theta)) * \
        BACK_INSET + math.cos(math.radians(theta)) * (WIDTH / 2)
    y_right_back = y - math.cos(math.radians(theta)) * \
        BACK_INSET - math.sin(math.radians(theta)) * (WIDTH / 2)
...
def plot_instr(init_pos, instr_list, obstacles):
    ...
    for idx, instr in enumerate(instr_list):
        ...
        if instr[3:5] == 'DM':
            index = int(instr[5])
            distances[index] = distance_to_collision(
                cur_pos, obstacles) - int(instr[6:])
            step_instr = 'PS|FW001'
...
def plot_obstacles(obstacles):
    ...
    for obstacle in obstacles:
        ...
        if obstacle[2] == 0:
            plt.plot([obstacle[0], obstacle[0] + 1],
                     [obstacle[1] + 1, obstacle[1] + 1], color='red')
        elif obstacle[2] == 1:
            plt.plot([obstacle[0], obstacle[0] + 1],
                     [obstacle[1], obstacle[1]], color='red')
        elif obstacle[2] == 2:
            plt.plot([obstacle[0], obstacle[0]], [
                     obstacle[1] + 1, obstacle[1]], color='red')
        elif obstacle[2] == 3:
            plt.plot([obstacle[0] + 1, obstacle[0] + 1],
                     [obstacle[1] + 1, obstacle[1]], color='red')
    ...
    for idx_x in range(x_lim):
        for idx_y in range(y_lim):
            if collided_cells[idx_x][idx_y]:
                ax.add_patch(Rectangle((idx_x, idx_y), 1,
                                       1, color='blue', alpha=0.06))
